...
inline typename vertex_buffer<Scalar, Target>::ptr vertex_buffer<Scalar, Target>::from_data(const C<Scalar, A<Scalar>>& sequence) {
    return vertex_buffer<Scalar, Target>::from_data(sequence.data(), sequence.size());
}

template <typename Scalar, GLenum Target>
template <template <typename, typename> class C, typename S, template <typename> class A>
inline typename vertex_buffer<Scalar, Target>::ptr vertex_buffer<Scalar, Target>::from_data(const C<S, A<S>>& sequence) {
    C<Scalar, A<Scalar>> cast_copy(sequence.begin(), sequence.end());
    return vertex_buffer<Scalar, Target>::from_data(cast_copy.data(), cast_copy.size());
}

template <typename Scalar, GLenum Target>
template <int Dim, template <typename, typename> class C, template <typename> class A>
inline typename vertex_buffer<Scalar, Target>::ptr vertex_buffer<Scalar, Target>::from_data(const C<eigen_vector<Scalar, Dim>, A<eigen_vector<Scalar, Dim>>>& vector_sequence) {
    ...
    return vertex_buffer<Scalar, Target>::from_data(matrix);
}

template <typename Scalar, GLenum Target>
template <int Dim, template <typename, typename> class C, typename S, template <typename> class A>
inline typename vertex_buffer<Scalar, Target>::ptr vertex_buffer<Scalar, Target>::from_data(const C<eigen_vector<S, Dim>, A<eigen_vector<S, Dim>>>& vector_sequence) {
    Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic> matrix(vertex_sequence.size(), Dim);
    for (uint32_t i = 0; i < vector_sequence.size(); ++i) {
...
inline typename vertex_buffer<Scalar, Target>::ptr vertex_buffer<Scalar, Target>::from_data(const Eigen::Matrix<Scalar, Rows, Cols, Options>& matrix) {
    ...
    return vertex_buffer<Scalar, Target>::from_data(data_transposed.data(), matrix.rows() * matrix.cols());
}

template <typename Scalar, GLenum Target>
template <typename S, int Rows, int Cols, int Options>
inline typename vertex_buffer<Scalar, Target>::ptr vertex_buffer<Scalar, Target>::from_data(const Eigen::Matrix<S, Rows, Cols, Options>& matrix) {
