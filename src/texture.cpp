...
texture::ptr texture::depth_texture(int width, int height, GLenum format) {
    ...
    return ptr(new texture(scalar_type, GL_DEPTH_COMPONENT, format, width, height, 0, (const Scalar*)nullptr, params, true));
}
...
void texture::resize(int width, int height, int depth) {
	...
	depth_ = depth;
	switch (dims) {
		case 1: glTexImage1D(target_, 0, internal_format_, width_, 0, internal_layout_, scalar_type_, nullptr); break;
		case 2: glTexImage2D(target_, 0, internal_format_, width_, height_, 0, internal_layout_, scalar_type_, nullptr); break;
		case 3: glTexImage3D(target_, 0, internal_format_, width_, height_, depth_, 0, internal_layout_, scalar_type_, nullptr); break;
		default: break;
	}
	release();
}
...
void texture::set_data(const Scalar* data) {
    bind();
	switch (dims_) {
        case 1: glTexImage1D(target_, 0, internal_format_, width_, 0, internal_layout_, scalar_type_, (char*)data); break;
        case 2: glTexImage2D(target_, 0, internal_format_, width_, height_, 0, internal_layout_, scalar_type_, (char*)data); break;
        case 3: glTexImage3D(target_, 0, internal_format_, width_, height_, depth_, 0, internal_layout_, scalar_type_, (char*)data); break;
		default: break;
	}
    release();
}
...
template <typename Scalar>
void texture::get_data(Scalar* data) {
    bind();
	switch (dims_) {
		case 1: glGetTexImage(target_, 0, internal_layout_, scalar_type_, (void*)data); break;
		case 2: glGetTexImage(target_, 0, internal_layout_, scalar_type_, (void*)data); break;
		case 3: glGetTexImage(target_, 0, internal_layout_, scalar_type_, (void*)data); break;
		default: break;
	}
    release();
}
...
template <typename Scalar>
texture::texture(GLenum scalar_type, GLenum internal_layout, GLenum internal_format, int width, int height, int depth, const Scalar* data, parameters_t_ params, bool is_depth_attachment) :
	width_(width),
	height_(height),
	depth_(depth),
	scalar_type_(scalar_type),
	internal_layout_(internal_layout),
...
#define TEXTURE_SCALAR_INSTANTIATE(type) \
	template texture::ptr texture::texture_1d(int width, int channels, const type* data, boost::optional<GLenum> format, GLenum min_filter, GLenum mag_filter, GLenum wrap_s, GLenum wrap_t); \
	template texture::ptr texture::texture_2d(int width, int height, int channels, const type* data, boost::optional<GLenum> format, GLenum min_filter, GLenum mag_filter, GLenum wrap_s, GLenum wrap_t); \
	...
	template texture::texture(GLenum scalar_type, GLenum internal_layout, GLenum internal_format, int width, int height, int depth, const type* data, parameters_t_ params, bool is_depth_attachment); \
	template void texture::set_data(const type* data); \
	template void texture::get_data(type* data);
